...
template <typename T>
void dump_ac(int frame, const T& array)
{
  char buf[256];
  sprintf(buf, "active_coeffs%06d.h5", frame);

  hid_t h5f = H5Fcreate(buf, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
  eigen2hdf::save(h5f, "C", array);
  H5Fclose(h5f);
}
    ...
    if (!rf_ptr) {
      ...
      if (vm.count("alld")) {
        ...
        *directions_ptr = load_directions_all(K);
        ...
        // make all directions
        if (vm.count("errors")) {
          std::cerr << "computing errors without using all directions is not implemented!"
                    << "\n";
          return 1;
        }
      ...
      std::cout << "what is inside dirs"
                << "\n";
      unsigned int nc = ncoeffs(*rf_ptr);
      active_coeffs.resize(qs.size(), nc);
...
#pragma omp critical
    ...
    if (vm.count("save-all")) {
      dump_ac(frame_numbers[fc], active_coeffs);
    } else if (vm.count("save") && (fc == 0 || fc == fnames.size() - 1)) {
      dump_ac(frame_numbers[fc], active_coeffs);
    }

    fc++;  // increment frame counter
  }        // end for files
  ...
  //   char buf[256];
  //   sprintf(buf, "active_coeffs%06d-%06d.h5", frame_min, frame_max);

  //   hid_t h5f = H5Fcreate(buf, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
  //   eigen2hdf::save(h5f, "C", active_coeffs);
  //   H5Fclose(h5f);
  // }
